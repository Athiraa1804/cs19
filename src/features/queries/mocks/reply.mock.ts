...
// Session-scoped replies added when interns submit new replies
const sessionReplies: Reply[] = [];

export function addSessionReply(reply: Reply) {
  sessionReplies.push(reply);
}

export function clearSessionReplies() {
  sessionReplies.length = 0;
}

export function getSessionReplies(): Reply[] {
  return [...sessionReplies];
}

export function getRepliesForQuery(queryId: string): Reply[] {
  return [...mockReplies, ...sessionReplies].filter((r) => r.queryId === queryId);
}