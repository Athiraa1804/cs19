...
function RoleRoute({ allowed, children }: { allowed: AllowedRole; children: ReactNode }) {
  ...
  if (allowed === 'intern' && !admin) return <>{children}</>;
  if (allowed === 'admin' && admin) return <>{children}</>;

  navigate(admin ? '/admin/queries' : '/faqs', { replace: true });
  return null;
}
...
function NavigateToHome() {
  const navigate = useNavigate();
  navigate(isAdmin() ? '/admin/queries' : '/queries/raise', { replace: true });
  return null;
}
...
function NotFound() {
  return (
    <div className="max-w-lg mx-auto px-4 py-16 text-center min-w-0">
      ...
      <Link
        to={isAdmin() ? '/admin/queries' : '/queries/raise'}
      ...
      >
        {isAdmin() ? 'Go to Query Review' : 'Go to Raise Query'}
      </Link>
    </div>
  );
}
